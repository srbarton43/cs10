 ...
 */
public class SinglyLinked1<T> implements SimpleList<T> {
	...
	/**
	 * Move the Element at index idx to the front of the list
	 * @param idx - index of item to move to front
	 * @throws Exception
	 */

	public void moveToFront(int idx) throws Exception
	{
		if(size < 1 || idx >= size) throw new Exception();
		if(idx == 0) return;
		Element e = advance(idx);
		remove(idx);
		e.next = head;
