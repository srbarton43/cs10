...
public class DotTreeGUI extends DrawingGUI {
	...
	private int mouseRadius = 30;					// circle around mouse location, for querying
	...
	public void handleMousePress(int x, int y) {
		if (mode == 'a') {
			...
			if(tree==null) tree = new PointQuadtree<Dot>(new Dot(x, y), 0, 0, width, height); // if no tree create one
			else tree.insert(new Dot(x, y)); // else insert new dot at mouse press
		}
		else if (mode == 'q') {
			// Set "found" to what tree says is near the mouse press
			if(tree != null) found = tree.findInCircle(x, y, (mouseRadius+dotRadius)); // cr is sum of mouseRadius and dotRadius
		}
		else {
	...
	private void test2() { // test for multiple dots on same locationn
		found = null;
		tree = new PointQuadtree<Dot>(new Dot(400,300), 0,0, width, height);
		tree.insert(new Dot(400,300));
		tree.insert(new Dot(400,300));
		tree.insert(new Dot(400,300));
		tree.insert(new Dot(400,300));
		int bad = 0;
		bad += testFind(400,300,10,5,5,5); // find all
		if(bad==0) System.out.println("test 2 passed!");
	}

	private void test3() { // test for invalid inserts also for when the dot is on the edge of a boundary
		found = null;
		tree = new PointQuadtree<Dot>(new Dot(400,300), 0,0, width, height);
		tree.insert(new Dot(0,0));
		tree.insert(new Dot(800,600)); // invalid
		tree.insert(new Dot(500,200));
		tree.insert(new Dot(200,500));
		tree.insert(new Dot(200, 550)); // test on x2
		tree.insert(new Dot(-200,500)); // invalid
		int bad = 0;
	...
	public void drawTree(Graphics g, PointQuadtree<Dot> tree, int level) {
		...
		Dot p = tree.getPoint();
		g.fillOval((int)p.getX()-dotRadius, (int)p.getY()-dotRadius, dotRadius*2, dotRadius*2); // draws oval
		g.drawLine((int)p.getX(), tree.getY1(), (int)p.getX(), tree.getY2()); // quadrant lines
